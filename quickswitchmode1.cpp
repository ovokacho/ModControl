...
void quickSwitchMode1::toolcomboselected()
{
    ...
    while ( QLayoutItem* item = ui->verticalLayoutFv_1->takeAt(0) )
       {
           Q_ASSERT( ! item->layout() ); // otherwise the layout will leak
           delete item->widget();
           delete item;
       }
    while ( QLayoutItem* item = ui->verticalLayoutAtt_1->takeAt(0) )
       {
           Q_ASSERT( ! item->layout() ); // otherwise the layout will leak
           delete item->widget();
           delete item;
       }
    while ( QLayoutItem* item = ui->verticalLayoutOther_1->takeAt(0) )
       {
           Q_ASSERT( ! item->layout() ); // otherwise the layout will leak
           delete item->widget();
           delete item;
